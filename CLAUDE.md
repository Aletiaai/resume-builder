...
**Rate limit handling:** `llm_service.call()` automatically retries up to 3 times on a 429 `ResourceExhausted` response. Before retrying it checks whether the error message contains `"PerDay"` (the `GenerateRequestsPerDayPerProjectPerModel` quota violation string) — if so, it raises `GeminiDailyQuotaExhaustedError` immediately with no retry, since the daily cap cannot be resolved by waiting. For per-minute limits it parses the suggested retry delay (`"Please retry in Xs"`) and waits that duration, falling back to exponential backoff (30s → 60s → 120s) if the delay cannot be parsed. After all retries are exhausted it raises `GeminiQuotaExhaustedError`.

**Typed LLM exceptions** — `llm_service.py` defines three custom exception classes that propagate to the orchestrator:
- `GeminiDailyQuotaExhaustedError` — raised immediately when the `"PerDay"` marker is found in a 429 error. No retry.
- `GeminiQuotaExhaustedError` — raised when a per-minute `ResourceExhausted` survives all retries.
- `GeminiInvalidKeyError` — raised immediately on `InvalidArgument`, `PermissionDenied`, or `Unauthenticated` (bad or revoked key).

The orchestrator catches each type and stores a specific status on the generation record: `failed_quota_daily`, `failed_quota`, `failed_key`, `failed_timeout` (5-minute hard timeout enforced by `asyncio.wait_for` in the background task wrapper), or `failed` (generic). The polling endpoint maps these to an `error_code` field in the response so the frontend can show the right Spanish message. Never catch these exceptions in agents — let them propagate to the orchestrator.
...
4. If FAIL → Repair Agent
   ...
   - **Batched by section:** one LLM call per distinct flagged section (not one call for all findings combined). Each call receives only the current section value + findings for that section. Combined usage metadata is returned.
   - Output: corrected section(s) patched back into the resume
...
**Tailor agent JSON robustness:** `tailor_agent.py` uses `_clean_json()` before parsing: strips markdown code fences, extracts the outermost `{…}` block, and removes trailing commas (a common Gemini formatting quirk). If `json.loads` still fails, the call is retried once with the same prompt (`_MAX_PARSE_RETRIES = 1`) before raising `RuntimeError`. The tailor agent is the only one with a parse-failure retry because without a valid resume there is nothing to deliver.
...
**Validation failure highlighting — granular per item:**
- `generate_docx()` receives `flagged_findings: list[dict]` (each dict has `section` and `flagged_text`) instead of just a list of section names.
- For non-experience sections (summary, skills, education, languages): the whole section paragraph is highlighted red.
- For experience: each job title line and each bullet is checked individually against `flagged_text` using bidirectional substring matching (`_text_is_flagged()`). Only the specific matching element is highlighted — unflagged titles and bullets in the same job entry remain unstyled. The section header ("RELEVANT WORK EXPERIENCE") is never highlighted.
- The Spanish warning comment is added only to the specific flagged element, not to the entire job entry.
- Skills section renders `**Category**:` markdown as bold text runs via `_render_bold_markdown()`, which splits on `**` and alternates bold/normal runs.
- Filename format: `FirstName-LastName-resume-MonYY-CO.docx` (English) / `FirstName-LastName-CV-MonYY-CO.docx` (Spanish)
...
- `resume_first_name` — first name(s) — required
- `resume_last_name` — last name(s) — required
...
**Contact info in the tailor agent:** `resume.py` extracts all 6 fields from the user record and passes them as `contact_info` to the orchestrator → tailor agent. The `TAILOR_USER_PROMPT_TEMPLATE` injects them as authoritative values and explicitly instructs the model to copy them verbatim — `candidate_name` must be exactly `"{first_name} {last_name}"` and `contact_line` must use only City, Phone, Email, LinkedIn. The orchestrator additionally overrides `tailored_resume.candidate_name` after the tailor agent returns to guarantee the profile name is used, regardless of what the model inferred.

**Validator bypass:** `VALIDATOR_USER_PROMPT_TEMPLATE` includes an explicit instruction not to validate or flag `candidate_name` or `contact_line`, since both come from verified profile data rather than the uploaded resume.

**`has_profile` flag:** computed in `GET /auth/me` as `bool(first_name and last_name and city and phone and resume_email)`. The frontend uses this to decide whether to show `screen-profile` during bootstrap.
...
- Frontend polling (`startPolling` in `app.js`): 5-second interval, 10-minute ceiling (`MAX_TOTAL_POLLS = 120`), 60-second consecutive-failure window (`MAX_POLL_FAILURES = 12`). An `isPollPending` flag prevents overlapping concurrent requests — a tick is skipped if the previous fetch has not yet resolved. The failure counter resets to 0 on every successful response regardless of generation status. `completed` and `failed` statuses are handled as explicit early returns (not a shared else branch).
